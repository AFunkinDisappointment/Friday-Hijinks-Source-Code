...
class PlayState extends MusicBeatState
{
	...
	function makeHaxeState(usehaxe:String, path:String, filename:String) {
		...
		interp.variables.set("doof", doof);
	...
	function makeHaxeStateUI(usehaxe:String, path:String, filename:String) {
		...
		//funny numbers (I SWEAR TO GOD DON'T WRITE TO THEM)
	...
	override public function create()
	{
		...
		*/
		...
		classicUI = OptionsHandler.options.classicUI;
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				*/
				...
				if (swagNote.nukeNote && SONG.uiType == 'tricky-death') {
					swagNote.x -= 165;
				}
				if (swagNote.mineNote && SONG.uiType == 'tricky-fire') {
					swagNote.x -= 50;
				}
	...
	override public function update(elapsed:Float)
	{
		...
		if (((health <= 0 && !opponentPlayer) || (health >= 2 && opponentPlayer)) && !practiceMode && !duoMode)
		{
			...
			if (inALoop) {
				...
				else if (SONG.stage == 'haunted-mansion') {
					LoadingState.loadAndSwitchState(new GameOverSpecialState());
				}
				else
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if ((!duoMode && !opponentPlayer) || demoMode) {
					enemyStrums.forEach(function(spr:FlxSprite)
					{
						...
						if (spr.animation.curAnim != null && spr.animation.curAnim.name == 'confirm' && !daNote.isPixel && uiSmelly.uses != 'bf')
						{
							spr.centerOffsets();
							spr.offset.x -= 13;
							spr.offset.y -= 13;
