...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var hitsounds = new FlxUICheckBox(10, 170, null, null, "Play hitsounds", 100);
	...
	function addCharsUI():Void
	{
		...
		stageID = new FlxUINumericStepper(100, 180, 1, _song.stageID, 0, 999, 0);
	...
	function addNoteUI():Void
	{
		...
		/*stepperNoteType = new FlxUINumericStepper(100, 200, 1, 0, 0, 999, 0);
	...
	override function update(elapsed:Float)
	{
			...
			curRenderedNotes.forEach(function(note:Note)
			{
				if (FlxG.sound.music.playing)
				{
					FlxG.overlap(strumLine, note, function(_, _)
					{
						...
						{
							claps.push(note);
							FlxG.sound.play('assets/sounds/SNAP.ogg');
						}
					});
				}
			});
		...
		if (controls.TERTIARY) {
			...
			switch(noteAddValue) {
			    case 8:
					trace('mine note activated');
				case 16:
					trace('lift note activated');
				case 24:
					trace('death note activated');
				case 32:
					trace('drain note activated');
				case 40:
					trace('normal note activated');
					noteAddValue = 0;
			}
		}
	...
	private function addNote():Void
	{
		...
		_song.notes[curSection].sectionNotes.push([noteStrum, noteData + noteAddValue, noteSus, false, useLiftNote]);
