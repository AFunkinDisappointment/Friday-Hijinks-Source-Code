...
class FreeplayState extends MusicBeatState
{
	...
	//var scoreTextTab:FlxText;
	//var accuracyTextTab:FlxText;
	var lerpScore:Int = 0;
	...
	override function create()
	{
		...
		/*var tabs = [
			...
			{name: "Random Song", label: 'Random Song'}
		];
		...
		if (!soundTest /*&& OptionsHandler.options.style*/) // Bruh, how do I see my FC if I'm in classic -_-
			add(infoPanel);
		changeSelection();
		changeDiff();
	...
	/*
	function addScoreUI() {
		scoreTextTab = new FlxText(10, 100, 0, "", 16);
		scoreText.setFormat("assets/fonts/vcr.ttf", 16, FlxColor.WHITE, RIGHT);

		accuracyTextTab = new FlxText(10, 100, 0, "", 16);
		accuracyTextTab.setFormat("assets/fonts/vcr.ttf", 16, FlxColor.WHITE, RIGHT);

		var tab_group_score = new FlxUI(null, UI_box);
		tab_group_score.name = "Score";

		tab_group_score.add(scoreTextTab);
		tab_group_score.add(accuracyTextTab);

		UI_box.addGroup(tab_group_score);
	}

	function addRandomSongUI() {
	...
	*/
	override function update(elapsed:Float)
	{
        ...
        //scoreTextTab.text = "Score: " + lerpScore;
		//accuracyTextTab.text = "Accuracy: " + lerpAccuracy + "%";
		if (!soundTest)
		...
		if (accepted)
		{
			...
			else
			{
				...
				trace(poop);
				if (songs[curSelected].songName.toLowerCase() != 'random-song' && !FNFAssets.exists('assets/data/' + songs[curSelected].songName.toLowerCase() + '/' + poop.toLowerCase() + '.json'))
				{
					...
					curDifficulty = DifficultyIcons.getDefaultDiffFP();
				} else if (songs[curSelected].songName.toLowerCase() == 'random-song' && !FNFAssets.exists('assets/data/' + songs[randomValue].songName.toLowerCase() + '/' + poop.toLowerCase() + '.json')) {
					// assume we pecked up the difficulty, return to default difficulty
	...
	function changeSelection(change:Int = 0)
	{
		...
		FlxG.sound.play(FNFAssets.getSound('assets/sounds/custom_menu_sounds/'+CoolUtil.parseJson(FNFAssets.getText("assets/sounds/custom_menu_sounds/custom_menu_sounds.json")).customMenuScroll+'/scrollMenu' + TitleState.soundExt), 0.4);
